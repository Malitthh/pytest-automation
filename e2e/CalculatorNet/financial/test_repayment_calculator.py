...
MONTHS_ONLY_CASES = _DATA["months_only_cases"]
...
class TestRepaymentPositiveCases(BaseTest):
    ...
    @pytest.mark.parametrize("scenario", MONTHS_ONLY_CASES, ids=[s["id"] for s in MONTHS_ONLY_CASES])
    def test_monthly_payment_for_months_only_term(self, repayment_page, scenario):
        repayment_page.fixed_time_calculate(
            scenario["loan_amount"],
            scenario["annual_interest_rate"],
            scenario["years"],
        ...
        expected = scenario["expected_monthly"]
        expect(repayment_page.monthly_payment_cell(expected)).to_be_visible()
